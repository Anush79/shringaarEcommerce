...
export default function () {
  ...
  console.log(prodID)
  const product = backendData?.productsData.find((item) => item.product_id === prodID)
  if (product) {
    ...
    return <div className="productDetailsContainer">
      ...
      <div className="detailsContainer" >
        ...
        <div className="textContentContainer">
          <h2>{product_name}</h2>
          <small>{product_category} by </small>{product_brand}

          <div className="offer">Buy 2 & get 5% cashback</div>
          <div className="price">$ {product_price} <span className="stikeThrough">$ {product_prevPrice}</span>
            <span className="discount">({discount}% off)</span></div>
  ...
  } else return <h2 style={{ height: "80vh", marginTop: "100px" }}>sorry, We could not find that product</h2>
